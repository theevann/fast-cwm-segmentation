...
from models import DeepLabCWM, Stepper, MaskGenerator
...
class TrainerCWM(Trainer):
    def get_model_and_params(self, num_classes):
        ...
        model = DeepLabCWM(num_classes=num_classes,
            backbone=self.args.backbone,
            output_stride=self.args.out_stride,
            norm_layer=None,
            freeze_bn=False,
            image_lvl_feat=not self.args.no_image_lvl_feat
        )
        ...
        if self.args.pretrained is not None:
            ...
            train_params = [
                ...
                {'params': model.fine_tune_params(), 'lr': self.args.lr / 10, 'weight_decay': self.args.weight_decay / 10}
            ]
